...
function db_affected_rows ($db_link=false)
{
	...
	if (!$db_link || $db_link->connect_errno)
	{
		// Возврат ошибки
		return false;
...
function db_connect ($connection_string, $fglobal=true, $fpool=false)
{
	...
	else
	{
		...
		if ($db_link && !$db_link->connect_errno) db_set_encoding($params['codepage'], $db_link);
...
// Функция получения номера последней ошибки обращения к БД
function db_error_code ($db_link=false)
{
	global $_db;
	...
	{
		// Возврат кода ошибки
		return $db_link->connect_errno;
		
	}
	
	// Возврат кода последней ошибки обращения к БД
	return $db_link->errno;
	
}


// Функция получения текста последней ошибки обращения к БД
